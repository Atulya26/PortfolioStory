...
export function useCaseStudyMotion(rootRef: React.RefObject<HTMLElement | null>) {
  useGSAP(() => {
    ...
    gsap.utils.toArray<HTMLElement>('[data-stagger]').forEach((group) => {
      ...
      gsap.from(items, { y: 34, autoAlpha: 0, scale: 0.992, duration: 0.82, ease: 'power3.out', stagger: 0.075, scrollTrigger: { trigger: group, start: 'top 86%' } });
    });

    gsap.utils.toArray<HTMLElement>('[data-rise]').forEach((el) => {
      if (claimed.has(el)) return;
      gsap.from(el, { y: 30, autoAlpha: 0, duration: 0.78, ease: 'power3.out', scrollTrigger: { trigger: el, start: 'top 88%' } });
    });
